      ...
      <section className="md:py-20 py-10 px-4 overflow-hidden">
        <div className="container md:mx-auto md:max-w-6xl">
          <div className="grid md:grid-cols-2 gap-12 items-center w-full">
            <ScrollAnimate index={0} className="flex flex-col justify-center text-center md:text-left">
        ...
        <div className="container mx-auto">
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-8 mb-16">
            {services.map((service, index) => (
              <ScrollAnimate key={service.title} index={index} className="flex justify-center">
                <ServiceCard {...service} />
              </ScrollAnimate>
            ))}
          </div>
          ...
          <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
            {[africanStudents, africanTrade, africanNetworking].map((img, index) => (
              <ScrollAnimate key={index} index={index} className="flex justify-center">
                <div className="relative h-64 rounded-2xl overflow-hidden shadow-elegant group w-full max-w-sm">
                  ...
                  <div className="absolute inset-0 bg-gradient-to-t from-black/70 to-transparent flex items-end p-6">
                    <h3 className="text-white text-xl font-bold">
                      {index === 0 ? "Education Excellence" : index === 1 ? "Trade & Commerce" : "Global Partnerships"}
                    </h3>
                  </div>
                </div>
              </ScrollAnimate>
        ...
        <div className="container mx-auto max-w-6xl">
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-8">
            {corePriorities.map((priority, index) => (
              <ScrollAnimate key={priority.title} index={index} className="flex justify-center">
          ...
          <ScrollAnimate index={0} className="flex justify-center">
            <img
              ...
              className="rounded-2xl shadow-elegant w-full h-[400px] object-cover max-w-4xl mt-16"
            />
          </ScrollAnimate>
        </div>
      </section>
          ...
          <div className="container mx-auto max-w-4xl">
            <div className="bg-gradient-to-r from-primary to-primary-glow rounded-3xl p-12 text-center shadow-glow">
              ...
              <div className="flex flex-col sm:flex-row gap-4 justify-center">
                <Link to="/contact">
                  <Button size="lg" variant="outline" className="text-primary text-lg px-8">
                    Get in Touch
                  </Button>
                </Link>
                <Link to="/services">
                  <Button size="lg" className="border-white border bg-transparent text-white hover:text-blue-600 hover:bg-white text-lg px-8">
